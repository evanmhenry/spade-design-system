...
const DataTable = ({ title, data, columns, searchFilter = [], addLabel, onAdd, onRowClick, addDisabled, columnsEnabled }: DataTableProps) => {
	...
	const isOnePage = table.getPageCount() <= 1
	const NUM_DISPLAYING_CURRENTLY = table.getRowModel().rows.length
		...
		<div className='w-full'>
			<div className='flex items-center pb-4 justify-between'>
				...
				<div className='flex items-center space-x-2'>
					...
					{onAdd && (
						<Button onClick={onAdd} variant='brand' disabled={addDisabled}>
			...
			<div className='flex items-center justify-end space-x-2 py-4'>
				<div className='flex-1 text-sm text-muted-foreground'>
					Showing: <span className='font-semibold'>{NUM_DISPLAYING_CURRENTLY}</span> of {TOTAL_ENTRIES}
				</div>
				{!isOnePage && (
