...
const MenuButton = ({ isOpen, setIsOpen }: { isOpen: boolean; setIsOpen: React.Dispatch<React.SetStateAction<boolean>> }) => (
	<button onClick={() => setIsOpen(!isOpen)} className='flex flex-col w-6 h-6 justify-center items-center gap-2'>
		<div className={`w-5 h-[2px] rounded bg-gray-800 transition-transform duration-200 ${isOpen ? 'transform rotate-45 translate-y-1.5' : ''}`}></div>
		<div className={`w-5 h-[2px] rounded bg-gray-800 transition-transform duration-200 ${isOpen ? 'transform -rotate-45 -translate-y-1' : ''}`}></div>
	</button>
)
...
const LandingPage = () => {
	...
	return (
		<div className='h-dvh'>
			<header className={`relative flex flex-col container border-b items-center transition-all py-4 duration-200 ${isOpen ? 'h-dvh' : 'h-16'}`}>
				<div id='menu' className='flex justify-between w-full items-center'>
					<h1 className='text-2xl font-bold'>React Pattern</h1>
					<MenuButton isOpen={isOpen} setIsOpen={setIsOpen} />
				</div>
				<div className={`overflow-hidden transition-all duration-200 ${isOpen ? 'h-full' : 'max-h-0'} w-full gap-4 pt-6`}>
			...
			<div className='bg-white h-dvh'>
				<div className='max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8'>
					<div className='text-center'>
						<h2 className='text-3xl font-extrabold text-gray-900 sm:text-4xl'>Welcome to React Pattern</h2>
						<p className='mt-4 text-lg leading-6 text-gray-500'>This is an example project to demonstrate react conventions.</p>
						<div className='mt-6'>
							<Link to='/buttons'>
